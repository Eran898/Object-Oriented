		...
		btnCalcLocation.addSelectionListener(new SelectionAdapter()
		{
			...
			public void widgetSelected(SelectionEvent e)
			{
				try {
					...
					if (args.length == 1)
					{
						CalcLocation CL = new CalcLocation(FL.findLocationsOfMAC(args[0]));
						System.out.println(Arrays.toString(CL.calcLocation()));
					}
					else if (args.length == 2)
					{
						CalcLocation CL = new CalcLocation(FL.findLocationsOfMAC(args[0]));
						System.out.println(Arrays.toString(CL.calcLocation(Integer.parseInt(args[1]))));
					}
